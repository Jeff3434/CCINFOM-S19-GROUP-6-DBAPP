...
public class PetAdmissionApplication extends JFrame {
	...
	private final ButtonGroup btnGrpNewEx = new ButtonGroup();
	...
	public PetAdmissionApplication() {
		...
		rdbtnNew.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent e) {
		        ...
		        textFieldAge1.setEditable(true);
		...
		btnGrpNewEx.add(rdbtnExisting);
		rdbtnExisting.setFont(new Font("Segoe UI", Font.PLAIN, 12));
		rdbtnExisting.setBounds(150, 344, 70, 23);
		...
		btnApply.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent e) {
		        ...
		        String species = textFieldSpecies.getText().toLowerCase(); 
		        ...
		        if (rdbtnNew.isSelected()) {
		            ...
		            adopterAgeText = textFieldAge1.getText();
		            adopterSex = rdbtnMale_1.isSelected() ? "male" : "female";
		            status = rdbtnSingle.isSelected() ? "single" : "married";
		            contact = textFieldContact.getText();
		            reason = textAreaReason.getText();
		        	...
		        	try (Connection conn = dbConnector.getConnection()) {
		                ...
		                try (PreparedStatement stmt = conn.prepareStatement(sql)) {
		                    ...
		                    if (rs.next()) {
		                        ...
		                        adopterAgeText = String.valueOf(rs.getInt("age"));
		                        address = rs.getString("address");
		                        adopterSex = rs.getString("sex");
		                        status = rs.getString("civil_status");
		                        contact = rs.getString("contact_number");
		                        reason = rs.getString("reason");
		                    } else {
		                        JOptionPane.showMessageDialog(null, "Adopter not found.");
		                    }
		                }
		        ...
		        try (Connection conn = dbConnector.getConnection(); 
		            ...
		            if (rdbtnNew.isSelected()) {
		                ...
		                if (adopterRs.next()) {
		                    adopterId = adopterRs.getInt(1);
		                }
		            } else if (rdbtnExisting.isSelected()) {
		            	String selectedAdopter = choiceAdopter.getSelectedItem().toString();
		                String[] adopterParts = selectedAdopter.split(" - ");
		                adopterId = Integer.parseInt(adopterParts[0]);
		            }
